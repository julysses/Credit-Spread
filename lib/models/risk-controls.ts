...
export type DataConfidence = 'live' | 'delayed' | 'synthetic' | 'mock' | 'invalid';
...
export function buildDataQuality(sources: Record<string, string | undefined>, useMock = false): DataQuality {
  ...
  else if (values.some(v => v.includes('derived') || v.includes('synthetic') || v.includes('estimate'))) confidence = 'synthetic';
  else if (values.some(v => v.includes('delayed') || v.includes('yahoo') || v.includes('alpha'))) confidence = 'delayed';

  for (const [key, value] of Object.entries(normalized)) {
    if (/mock|hardcoded|fallback|derived|synthetic|estimate|unknown/i.test(value)) {
      warnings.push(`${key.toUpperCase()} source is ${value}`);
    }
  }

  return { confidence, warnings, sources: normalized };
}
