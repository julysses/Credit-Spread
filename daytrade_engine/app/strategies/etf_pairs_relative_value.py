...
class EtfPairsRelativeValueStrategy(BaseStrategy):
    name = StrategyName.ETF_PAIRS_RELATIVE_VALUE
    enabled = True
    required_regimes = {MarketRegime.RANGE, MarketRegime.MEAN_REVERTING}
    ...
    def scan(self, universe_state: dict[str, SymbolState], market_state: MarketState) -> list[SignalCandidate]:
        ...
        for symbol, state in universe_state.items():
            ...
            if not state.tradable:
                continue
            if state.asset_type.value=="ETF" and float(f.get("rvol",0))>1.0:
                out.append(
                    SignalCandidate(
                        ...
                        direction=Direction.PAIR_LONG_SHORT,
                        ...
                        explanation_seed=["pair dislocation", "correlation stable"],
                    )
                )
        return out
