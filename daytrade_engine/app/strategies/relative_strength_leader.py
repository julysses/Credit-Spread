...
class RelativeStrengthLeaderStrategy(BaseStrategy):
    name = StrategyName.RELATIVE_STRENGTH_LEADER
    enabled = True
    ...
    def scan(self, universe_state: dict[str, SymbolState], market_state: MarketState) -> list[SignalCandidate]:
        ...
        for symbol, state in universe_state.items():
            ...
            if not state.tradable:
                continue
            if float(f.get("return_vs_spy_intraday",0))>0.002 and float(f.get("rvol",0))>=1.5:
                out.append(
                    SignalCandidate(
                        ...
                        explanation_seed=["top RS percentile", "relative performance confirmed"],
                    )
                )
        return out
