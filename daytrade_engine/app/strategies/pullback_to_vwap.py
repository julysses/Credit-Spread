...
class PullbackToVWAPStrategy(BaseStrategy):
    name = StrategyName.PULLBACK_TO_VWAP
    enabled = True
    required_regimes = {MarketRegime.TREND_UP, MarketRegime.TREND_DOWN}
    ...
    def scan(self, universe_state: dict[str, SymbolState], market_state: MarketState) -> list[SignalCandidate]:
        ...
        for symbol, state in universe_state.items():
            ...
            if not state.tradable:
                continue
            if abs(float(f.get("distance_from_vwap_pct",0)))<0.003 and float(f.get("vwap_slope_5",0))>0:
                out.append(
                    SignalCandidate(
                        ...
                        explanation_seed=["controlled pullback", "VWAP support"],
                    )
                )
        return out
