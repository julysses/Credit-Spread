...
class MeanReversionExtensionStrategy(BaseStrategy):
    name = StrategyName.MEAN_REVERSION_EXTENSION
    enabled = True
    required_regimes = {MarketRegime.MEAN_REVERTING, MarketRegime.RANGE}
    ...
    def scan(self, universe_state: dict[str, SymbolState], market_state: MarketState) -> list[SignalCandidate]:
        ...
        for symbol, state in universe_state.items():
            ...
            if not state.tradable:
                continue
            if float(f.get("distance_from_vwap_pct",0))<-0.01:
                out.append(
                    SignalCandidate(
                        ...
                        explanation_seed=["extension below VWAP", "reversal context"],
                    )
                )
        return out
