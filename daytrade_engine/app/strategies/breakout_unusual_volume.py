...
class BreakoutUnusualVolumeStrategy(BaseStrategy):
    name = StrategyName.BREAKOUT_UNUSUAL_VOLUME
    enabled = True
    required_regimes = {MarketRegime.BREAKOUT_EXPANSION, MarketRegime.TREND_UP, MarketRegime.TREND_DOWN}
    ...
    def scan(self, universe_state: dict[str, SymbolState], market_state: MarketState) -> list[SignalCandidate]:
        ...
        for symbol, state in universe_state.items():
            ...
            if not state.tradable:
                continue
            if f.get("current_last",0) > f.get("prior_day_high",0) and float(f.get("rvol",0))>=2.0:
                out.append(
                    SignalCandidate(
                        ...
                        explanation_seed=["level breakout", "volume anomaly"],
                    )
                )
        return out
