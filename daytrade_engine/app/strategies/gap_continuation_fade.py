...
class GapContinuationFadeStrategy(BaseStrategy):
    name = StrategyName.GAP_CONTINUATION_FADE
    enabled = True
    required_regimes = {MarketRegime.BREAKOUT_EXPANSION, MarketRegime.MEAN_REVERTING}
    ...
    def scan(self, universe_state: dict[str, SymbolState], market_state: MarketState) -> list[SignalCandidate]:
        ...
        for symbol, state in universe_state.items():
            ...
            if not state.tradable:
                continue
            if abs(float(f.get("gap_pct",0)))>=0.0075:
                out.append(
                    SignalCandidate(
                        ...
                        explanation_seed=["meaningful opening gap", "OR behavior qualifies"],
                    )
                )
        return out
